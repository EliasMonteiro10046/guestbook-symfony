    ...
    */
    ...
    public function show(Request $request, Conference $conference, CommentRepository $commentRepository, ConferenceRepository $conferenceRepository, NotifierInterface $notifier, string $photoDir): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $notifier->send(new Notification('Obrigado pelo seu feedback; Seu comentário será publicado após ser verificado pela moderação', ['browser']));
            
            return $this->redirectToRoute('conference', ['slug' => $conference->getSlug()]);
        }

        if ($form->isSubmitted()) {
            $notifier->send(new Notification('Você pode verificar o seu envio? Existem alguns problemas com isso', ['browser']));
        }
